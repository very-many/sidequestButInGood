...
protected:
    ...
    SerialisableUser* createSerializeDeserializeUser(Id id, std::string email, std::string display_name,
                                                     std::string password) {
        auto user = new SerialisableUser("Unit Test User", "unitest@hs-aalende", "secret");
        user->id = 12;
