...
namespace Sidequest {
    ...
    RestTest::~RestTest() = default;

    void RestTest::SetUp() {
        //database = new Server::Database("unittest.db");
        // database = new Server::Database(":memory:");
        // database->execute_sql_statement(
        //     "CREATE TABLE user(id INTEGER PRIMARY KEY AUTOINCREMENT, email TEXT unique, display_name TEXT, password TEXT);");
        // database->execute_sql_statement(
        //     "CREATE TABLE quest(id INTEGER PRIMARY KEY AUTOINCREMENT, status TEXT, title TEXT, description TEXT, owner INTEGER, editor INTEGER, parent INTEGER);");

        std::string db_path = ":memory:";
        // std::string db_path = "../application_root/rest_list_test.db";
    ...
    SerialisableQuest* RestTest::create_subquest(SerialisableQuest* parent_quest, SerialisableUser* owner) {
        auto sub_quest = new SerialisableQuest( "Sub Quest", "", Quest::initial, parent_quest,owner, nullptr);
