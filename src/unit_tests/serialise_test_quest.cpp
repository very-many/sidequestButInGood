...
TEST_F(SerialiseTestsQuest, SERIALIZE_QUEST) {
    auto quest = new SerialisableQuest("Unit Test Quest",
                                       "Description\ncan be several lines\nor not.", Quest::Status::done, nullptr,
                                       nullptr, nullptr);
    quest->id = 12;
...
TEST_F(SerialiseTestsQuest, SERIALIZE_QUEST_PARENT) {
    auto quest = new SerialisableQuest("Unit Test Quest",
                                       ...
                                       nullptr, nullptr);
    quest->id = 12;
    auto parent_quest = new SerialisableQuest("Unit Test Main Quest", "", Quest::Status::done, nullptr, nullptr,
                                              nullptr);
...
TEST_F(SerialiseTestsQuest, SERIALIZE_QUEST_OWNER_EDITOR) {
    ...
    auto owner = new SerialisableUser("Unit Test User 1", "unitest1@hs-aalende", "secret");
    owner->id = 101;
    quest->owner = owner;
    quest->owner_id = quest->owner->id;

    auto editor = new SerialisableUser("Unit Test User 2", "unitest2@hs-aalende", "secret");
    editor->id = 102;
    quest->editor = editor;
